...
from models.hn_ppi import HNPPI
...
def train(class_tag, model, train_data_set, save, n_epochs=3,
    ...
    train_samples = sampler.SubsetRandomSampler(
        train_index)  # ========================================================================================
    ...
    # Optimizer
    optimizer = torch.optim.Adam(model_wrapper.parameters(), lr=0.0001)  # lr=0.0001)

    # Start log
    with open(os.path.join(save, 'HNPPI_results.csv'), 'w') as f:
        ...
        for epoch in range(n_epochs):
            ...
            if f_max > best_F:
                ...
                torch.save(model.state_dict(), os.path.join(save, 'HNPPI_model.dat'))
            else:
                count += 1
                if count >= 5:
                    return None
...
def demo(train_data, save=None, train_num=1,
    ...
    class_nums = 1
    model = HNPPI(class_nums, window_size, ratio)
...
if __name__ == '__main__':
    ...
    path_dir = "./checkpoints/hn_ppi_saved_models"
